...
#if defined(AUDIO_USE_IDF5_DRIVER) && \
...
AudioOutputI2S::AudioOutputI2S(long sampleRate, pin_size_t sck,
                               pin_size_t data) {
    i2sOn = false;
    mono = false;
    bps = 16;
...
AudioOutputI2S::~AudioOutputI2S() { stop(); }
...
bool AudioOutputI2S::SetPinout() {
    ...
    i2s_std_gpio_config_t pins = {.mclk = use_mclk ? (gpio_num_t)mclkPin
                                                   : I2S_GPIO_UNUSED,
...
bool AudioOutputI2S::begin(bool txDAC) {
    ...
    if (!i2sOn) {
        ...
        uint32_t rate = (uint32_t)AdjustI2SRate(this->hertz);
        if (output_mode == INTERNAL_PDM) {
    ...
    if (!i2sOn) {
        ...
        if (!txDAC) {
            audioLogger->printf_P(PSTR("I2SNoDAC: esp8266 arduino core should be "
                                       "upgraded to avoid conflicts with SPI\n"));
        }
#endif
    }
#elif defined(ARDUINO_ARCH_RP2040)
...
#endif
    i2sOn = true;
    SetRate(this->hertz); // Apply the desired clock rate
    return true;
}
...
bool AudioOutputI2S::ConsumeSample(int16_t sample[2]) {
    ...
    i2s_write((i2s_port_t)portNo, (const char *)&s32, sizeof(uint32_t),
              &i2s_bytes_written, 0);
